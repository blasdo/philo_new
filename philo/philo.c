...
/*   Updated: 2024/08/12 00:24:56 by bvelasco42       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	eat_odd(t_philo *this)
{
	...
	if (this->forks[0] == this->forks[1])
		return (usleep(cn->limit_time * 1000), p_log(this, DEAD));
	pthread_mutex_lock(this->forks[0]);
	p_log(this, FORK);
	pthread_mutex_lock(this->forks[1]);
	p_log(this, FORK);
	if (!verify_eat_time(this))
		p_log(this, DEAD);
	p_log(this, EAT);
	...
	// maybe can be created a funtion that waits the time to die if cannot wait eat_time
